...
export function CropBannerUploader({
  ...
  onRemove,
  label = "Banner Blog",
  description = "Upload banner blog (akan di-crop 16:9 dan diupload saat simpan)",
  ...
  const handleCropComplete = useCallback(
    (croppedBlob: Blob) => {
      ...
      const croppedFile = new File([croppedBlob], "banner-image.jpg", {
        type: "image/jpeg",
      });
        ...
        {imagePreview ? (
          <>
            <img
              src={imagePreview}
              alt="Preview"
              className="w-full aspect-video mx-auto rounded-lg object-cover"
              loading="lazy"
            />
            {hasPendingFile && (
